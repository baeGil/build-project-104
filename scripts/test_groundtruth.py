#!/usr/bin/env python3
"""
Ground Truth Contract Review Test

Tests the contract review pipeline with a contract designed to match
the 50 ingested documents (Sắc lệnh 1950s + Quyết định UBND 1999-2000).
...
async def run_groundtruth_test():
    """Run comprehensive ground truth test."""
    
    console.print("\n" + "=" * 80, style="bold cyan")
    console.print("🎯 GROUND TRUTH CONTRACT REVIEW TEST", style="bold cyan")
    console.print("=" * 80 + "\n")
    
    # Load contract
    console.print("📄 Loading contract...", style="dim")
    contract_text = load_contract("test contracts/test_contract_groundtruth.txt")
    ...
    expected = load_expected_output("EXPECTED_GROUNDTRUTH_OUTPUT.json")
    ...
    for i, clause in enumerate(clauses):
        ...
        # Debug: Show query expansion variants
        console.print(f"  📝 Query variants: {query_plan.expansion_queries[:3] if hasattr(query_plan, 'expansion_queries') else 'N/A'}")
        
        # Search for relevant laws, passing query_plan for expansion-variant boosting
        retrieval_start = time.perf_counter()
        
        try:
            # Hybrid retrieve with large candidate pool
            retrieval_start = time.perf_counter()
            
            # Step 1: Get large candidate pool for better recall
            search_results = await retriever.search(
                query_plan.normalized_query,
                query_plan=query_plan,
                top_k=20,  # Return top-20 to maximize recall (was 15)
                bm25_candidates=200,  # Larger pool for better recall (was 150)
                dense_candidates=200,  # Larger pool for better recall (was 150)
            ...
            # Show top 5 results (increased from 3 for better visibility)
            if search_results:
            ...
            # NOTE: In a full pipeline, LLM would analyze here
            # For this test, we'll simulate the finding based on retrieval
            finding = {
                "clause_number": i + 1,
                ...
                "retrieved_docs": [
                    ...
                    for doc in search_results[:20]  # Store all 20 results for maximum coverage analysis
                ],
                "status": "retrieved",
    ...
    output = {
        "test_metadata": {
            "test_name": "Ground Truth Contract Review Test",
            "contract_file": "test contracts/test_contract_groundtruth.txt",
    ...
    # Save to file
    output_file = Path(__file__).parent.parent / "test_groundtruth_output.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    
    console.print(f"\n💾 Detailed output saved to: test_groundtruth_output.json")
    ...
    avg_rerank = sum(perf_metrics["rerank_times"]) / len(perf_metrics["rerank_times"]) if perf_metrics["rerank_times"] else 0
    ...
    perf_table.add_row(
        "Avg Retrieval/Clause (incl. rerank)",
    ...
    # Retrieval quality
    console.print("\n" + "=" * 80, style="bold cyan")
    console.print("🔍 RETRIEVAL QUALITY", style="bold cyan")
    console.print("=" * 80 + "\n")
    ...
    console.print("1. Review test_groundtruth_output.json for detailed results")
    console.print("2. Compare retrieved law_ids with expected law_ids")
    console.print("3. Check if LLM review (if run) matches expected risk levels")
    console.print("4. Analyze retrieval quality and suggest improvements\n")
    ...
    console.print("[bold green]✅ Ground truth test complete![/bold green]\n")


async def main():
    """Run the test."""
    await run_groundtruth_test()
